---
name: sakhy-ecommerce-build
description: Project context and build spec for Sakhy, a premium heritage-saree ecommerce brand (storefront + admin panel + inventory + payments + security, ~100 orders/month). Use this skill whenever the user asks to build, scaffold, extend, secure, or review any part of the Sakhy site — storefront pages, product/catalog features, cart/checkout, the admin panel, inventory management, payment integration, database schema, authentication, or security/hardening (SQL injection, DDoS, rate limiting, RLS, webhook verification) — even if they don't say "Sakhy" by name but describe this saree ecommerce project. Also use it before generating any UI or backend code for this project, to stay consistent with the established design tokens, tech stack, data model, and security requirements rather than inventing new ones.
---
...
A client-supplied static HTML file was the original design reference. It is a **visual/motion reference only** — it has no cart, no checkout, no product data model, and hardcodes all content into one file. Never port its DOM structure or its manual content duplication (e.g. hand-copied marquee/testimonial blocks). Reuse only its design tokens and motion language, rebuilt as proper components.
